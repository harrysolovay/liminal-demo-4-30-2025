import { generateObject, generateText, jsonSchema, streamObject, streamText, tool as aiTool, } from "ai";
import { LiminalAssertionError, Model } from "liminal";
export function adapter(model) {
    return new Model("ai-sdk", ({ messages, schema: lSchema, signal, tools: lTools }) => {
        const tools = Object.fromEntries(lTools
            ?.values()
            .map((tool) => [
            tool.name,
            aiTool({
